...
Follow the `deepen` workflow defined in `AGENTS.md` for: $ARGUMENTS
...
2. Read only the named sections, or if none were named, the minimum sections needed
   to move one rung up the ladder: metadata -> abstract -> sections -> full. Never
   read the full PDF end-to-end unless the target depth is `full`.
3. Update **Method**, **Evidence**, and **Sections read** with facts only: what the
   paper contributes, claims, or shows. Do not add project-relevance judgments.
...
Report what changed: old depth -> new depth, which sections were read, and what was
added to the note.
