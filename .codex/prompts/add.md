...
Follow the `add` workflow defined in `AGENTS.md` for: $ARGUMENTS

The first token is the citekey or DOI/arXiv URL. An optional second token names
a topic map explicitly.

Steps:
1. If given a DOI/arXiv URL, fetch metadata: title, authors, year, venue, abstract.
...
3. If an abstract is available, fill in **Contribution** and **Key claims** from it
   with facts only and no relevance or priority judgments, then bump `depth: abstract`.
...
Report which files were created or changed, the resulting depth, and whether a
