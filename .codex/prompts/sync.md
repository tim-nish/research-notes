...
2. Read `.sync/queue.json`. Process up to the batch-size limit of `pending`
   ...
   - `action: create` -- first check whether `papers/<citekey>.md` already
     exists (it shouldn't, but never overwrite a note that's already there --
     treat that as a metadata conflict and report it instead of authoring).
     Otherwise author it from `templates/paper.md` following the `add`
     workflow's conventions (AGENTS.md): set `depth: metadata`, or
     `depth: abstract` (and fill **Contribution** + **Key claims** from
     `metadata.abstractNote`, facts only, no relevance judgments) when the
     queue entry's `metadata.abstractNote` is non-empty. **Must populate
     `zotero: zotero://select/library/items/<itemKey>`** -- this is the
     idempotency join future `/sync` runs rely on. The note is born
     `topics: []` -- `sync` never opens or edits `topics/*.md` (placement is
     `/propose`'s job, per specs/sync-placement.md).
